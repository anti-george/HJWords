...
class QuaZipFilePrivate {
  ...
  private:
        ...
        zipError(UNZ_OK)
    {
        ...
        if (this->fileName.startsWith('/')) {this->fileName = this->fileName.mid(1);}
...
void QuaZipFilePrivate::setZipError(int zipError) const
{
    ...
    if (zipError == UNZ_OK) {q->setErrorString(QString());}
    else {q->setErrorString(QString("ZIP/UNZIP API error") + zipError);}
}
...
bool QuaZipFile::open(OpenMode mode, int *method, int *level, bool raw, const char *password)
{
    ...
    if ((mode & ReadOnly) && not (mode & WriteOnly)) {
        if (p->internal)
        {
            if (not p->zip->open(QuaZip::mdUnzip))
            {
                p->setZipError(p->zip->getZipError());
                return false;
            }
            if (not p->zip->setCurrentFile(p->fileName, p->caseSensitivity))
            {
                p->setZipError(p->zip->getZipError());
                p->zip->close();
                return false;
    ...
    int windowBits, int memLevel, int strategy)
{
    ...
    if ((mode & WriteOnly) && not (mode & ReadOnly))
    {
        if (p->internal)
        {
        ...
        if (p->zip->getMode() != QuaZip::mdCreate && p->zip->getMode() != QuaZip::mdAppend
        ...
        else zipClearFlags(p->zip->getZipFile(), ZIP_WRITE_DATA_DESCRIPTOR);
...
bool QuaZipFile::atEnd() const
{
    ...
    if (openMode()&ReadOnly) {return QIODevice::bytesAvailable() == 0 && unzeof(p->zip->getUnzFile()) == 1;}
    else return true;
}
...
qint64 QuaZipFile::size()const
{
    ...
    if (openMode() & ReadOnly) {return p->raw ? csize() : usize();}
    else return p->writePos;
}
...
{
    p->resetZipError();
    if (p->zip == NULL || not p->zip->isOpen()) {return;}
    if (not isOpen())
    {
        qWarning("QuaZipFile::close(): file isn't open");
        return;
    }
    if (openMode() & ReadOnly) {p->setZipError(unzCloseCurrentFile(p->zip->getUnzFile()));}
